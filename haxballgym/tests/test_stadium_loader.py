...
def test_geometry_from_map():
    for name, gx, exp_pms in [("classic", 370.0, pms(0.10, 0.96)),
...
def test_classic_loaded_matches_builtin():
    ...
    a = hc.VecEnv(N, 1, 1, step_limit=SL, tick_skip=TS); a.reset_all()
    b = hc.VecEnv.from_hbs(stadium_text("classic"), N, 1, 1, step_limit=SL, tick_skip=TS); b.reset_all()
    md = 0.0
    for _ in range(20):
        eng = np.empty((N, 2, 3), np.int64)
        eng[..., 0] = rng.integers(-1, 2, (N, 2)); eng[..., 1] = rng.integers(-1, 2, (N, 2))
        eng[..., 2] = rng.integers(0, 2, (N, 2))
        a.physics_step(eng); b.physics_step(eng)
        pa, _, ppa, _, _ = a.snapshot(); pb, _, ppb, _, _ = b.snapshot()
...
def test_futsal_env_runs():
    ...
    obs = env.reset()
    for _ in range(50):
        acts = np.stack([np.random.randint(0, 3, (64, 2)), np.random.randint(0, 3, (64, 2)),
                         np.random.randint(0, 2, (64, 2))], -1)
