...
FN_BASE = os.path.join(
    ...
    "..", "..", "..", "..", "Ursinaxball",
    "ursinaxball", "modules", "physics", "fn_base.py",
)
...
def test_disc_disc():
    bad = 0
    for _ in range(N):
        pa = rng.uniform(-50, 50, 2); pb = rng.uniform(-50, 50, 2)
        va = rng.uniform(-10, 10, 2); vb = rng.uniform(-10, 10, 2)
        ...
        ra_pa, ra_pb, ra_va, ra_vb = hc.disc_disc(
            v(pa), v(pb), v(va), v(vb), ra, rb, ima, imb, ba, bb)
        if not (np.allclose(pa_p, ra_pa, atol=TOL) and np.allclose(pb_p, ra_pb, atol=TOL)
                and np.allclose(va_p, ra_va, atol=TOL) and np.allclose(vb_p, ra_vb, atol=TOL)):
            bad += 1
...
def test_disc_vertex():
    bad = 0
    for _ in range(N):
        pd = rng.uniform(-30, 30, 2); pv = rng.uniform(-30, 30, 2)
        vel = rng.uniform(-10, 10, 2); r = rng.uniform(5, 20)
...
def test_disc_plane():
    ...
    for _ in range(N):
        ...
        dist = rng.uniform(-200, 0); r = rng.uniform(5, 20)
        bd, bp = rng.uniform(0, 1), rng.uniform(0, 1)
        pd_p, vel_p = fb.resolve_disc_plane_collision_fn(pd.copy(), normal.copy(), vel.copy(), dist, r, bd, bp)
...
def test_segment_no_curve():
    ...
    for _ in range(N):
        pd = rng.uniform(-200, 200, 2)
        v0 = rng.uniform(-200, 200, 2); v1 = rng.uniform(-200, 200, 2)
        ...
        if dist_p is None:
            ...
            if res is None or not np.isclose(dist_p, res[0], atol=TOL) or not np.allclose(normal_p, res[1], atol=TOL):
                bad += 1
...
def test_segment_curve():
    ...
    for _ in range(N):
        ...
        t0 = rng.uniform(-1, 1, 2); t1 = rng.uniform(-1, 1, 2)
        curve = rng.choice([-180, -90, 90, 180.0])
        dist_p, normal_p = fb.resolve_disc_segment_collision_curve_fn(pd.copy(), c.copy(), cr, t0.copy(), t1.copy(), curve)
...
if __name__ == "__main__":
    test_disc_disc(); print("disc_disc OK")
    test_disc_vertex(); print("disc_vertex OK")
    test_disc_plane(); print("disc_plane OK")
    test_segment_no_curve(); print("segment_no_curve OK")
    test_segment_curve(); print("segment_curve OK")
